...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 20 10:55:58 2022
...
from utils.utils import compute_overlap_between_freqMap_stimPoints
import re
...
def main():
    ...
    folders_of_int=['01_SBA_SENSORIAL']
    os.makedirs('maps_2mm_cleaned_and_thr',exist_ok=True)
    
    thr_dice=0.25
    thr_overlappoints_freq_maps=0.95
    ...
    for ii in folders_of_int:
        ...
        look_up_table=compute_look_up_table(seed_maps_no_duplicates_no_out)
        
        freq_map=np.mean(look_up_table,axis=0)
        
        #threshold_and_overlap=compute_overlap_between_freqMap_stimPoints(path_to_stim_points,freq_map)
        ...
        file_out_name=('maps_2mm_cleaned_and_thr/' 
                       + '_'.join(ii.split('_')[2:])+'_POSITIVE_no_thr_N_seeds_'
                       +str(len(seed_maps_no_duplicates_no_out))
                       +'_seeds.nii.gz'
                       )
        ...
        #freq_map[freq_map<threshold_and_overlap[np.where(threshold_and_overlap[:,1]>=thr_overlappoints_freq_maps)[0][-1],0]]=0

        
        #nii_=nib.Nifti1Image(freq_map, affine=aff, header=head)

        #file_out_name=('maps_2mm_cleaned_and_thr/' 
                       #+ '_'.join(ii.split('_')[2:])+'_POSITIVE_thr_'
                       #+ str(threshold_and_overlap[np.where(threshold_and_overlap[:,1]>=thr_overlappoints_freq_maps)[0][-1],0]) 
                       #+'_N_'
                       #+str(len(seed_maps_no_duplicates_no_out))
                       #+'_seeds.nii.gz'
                       #)
